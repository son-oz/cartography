...
@patch.object(cartography.intel.aws.iam, 'get_role_list_data', return_value=ROLES["Roles"])
@patch.object(cartography.intel.aws.ec2.instances, 'get_ec2_instances', return_value=INSTANCE_WITH_IAM_PROFILE)
def test_sync_iam_instance_profiles(mock_get_profiles, mock_get_roles, mock_get_instances, neo4j_session):
    """
    ...
    assert check_nodes(neo4j_session, 'AWSInstanceProfile', ['id', 'instance_profile_name']) == {
        ("arn:aws:iam::1234:instance-profile/cartography-service", "cartography-service"),
    }
